...
| Input | Action |
|---|---|
| X | Toggle favorite for the highlighted game |
...
Limitations: the filter matches by file name, and duplicate copies of the same game share a single entry (see [GameData.md](GameData.md)). A second copy of a favorite in another folder shows the heart, but only the most recently used copy passes the filter. ROM hacks share the game code of their base game, so they also share its favorite mark and statistics.
